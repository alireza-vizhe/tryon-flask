...
# تنظیمات پیش‌فرض واترمارک (پایین سمت چپ)
WATERMARK_WIDTH = 300
WATERMARK_HEIGHT = 150
PADDING_X = 10
PADDING_Y = 10
...
@app.route("/")
def hello_world():
    return "✅ Python OpenCV Service is Running on Render!"

def clean_specified_area(image_bytes, x_pad, y_pad, w, h):
    try:
        # تبدیل بایت به آرایه نامپای
        nparr = np.frombuffer(image_bytes, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        ...
        # محاسبه مختصات حذف واترمارک (گوشه پایین چپ)
        x_start = x_pad
        y_start = img_height - h - y_pad
        
        # جلوگیری از خروج از کادر
...
def process_image():
    ...
    try:
        # خواندن پارامترها (اختیاری)
        x = int(request.form.get('x', PADDING_X))
        y = int(request.form.get('y', PADDING_Y))
        w = int(request.form.get('width', WATERMARK_WIDTH))
        h = int(request.form.get('height', WATERMARK_HEIGHT))
